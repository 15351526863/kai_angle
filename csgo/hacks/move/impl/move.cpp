...
namespace hacks {
    ...
    void c_move::auto_strafe( valve::user_cmd_t& user_cmd ) const {
        ...
        if ( !( g_local_player->self( )->flags( ) & valve::e_ent_flags::on_ground ) ) {
			...
			if (hold_w) {
				if (hold_a)
					// Dividing by 2 will work fine after the cast
					wish_dir += (static_cast<float>(e_dirs::e_left) / 2.f);
				else if (hold_d)
					wish_dir += (static_cast<float>(e_dirs::e_right) / 2.f);
				else
					wish_dir += static_cast<float>(e_dirs::e_forwards);
			}
			else if (hold_s) {
				if (hold_a)
					wish_dir += static_cast<float>(e_dirs::e_back_left);
				else if (hold_d)
					wish_dir += static_cast<float>(e_dirs::e_back_right);
				else
					wish_dir += static_cast<float>(e_dirs::e_backwards);

				user_cmd.m_move.x() = 0.f;
			}
			else if (hold_a)
				wish_dir += static_cast<float>(e_dirs::e_left);
			else if (hold_d)
				wish_dir += static_cast<float>(e_dirs::e_right);
