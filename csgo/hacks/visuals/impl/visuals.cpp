...
*/
	...
	void c_visuals::on_create_move( const valve::user_cmd_t& cmd ) {
		...
		valve::g_engine_trace->trace_ray_(
			...
			static_cast<unsigned int>(_MASK_SOLID) | static_cast<unsigned int>(_CONTENTS_CURRENT_90), // <-- FIXED
	...
	void c_visuals::grenade_simulation_t::physics_push_entity( const sdk::vec3_t& push, valve::trace_t& trace ) {
		physics_trace_entity(m_origin, m_origin + push,
			m_collision_group == 1
			// FIX: Removed the stray comma and cast all enum members to unsigned int.
			? (static_cast<unsigned int>(_MASK_SOLID) | static_cast<unsigned int>(_CONTENTS_CURRENT_90)) & ~static_cast<unsigned int>(_CONTENTS_MONSTER)
			// FIX: Also cast all members in the "else" part of the expression.
			: static_cast<unsigned int>(_MASK_SOLID) | static_cast<unsigned int>(_CONTENTS_OPAQUE) | static_cast<unsigned int>(_CONTENTS_IGNORE_NODRAW_OPAQUE) | static_cast<unsigned int>(_CONTENTS_CURRENT_90) | static_cast<unsigned int>(_CONTENTS_HITBOX),
			trace
		);
		...
		if ( trace.m_start_solid ) {
			m_collision_group = 3;
			trace_line(m_origin - push, m_origin + push, trace, g_local_player->self(), (static_cast<unsigned int>(_MASK_SOLID) | static_cast<unsigned int>(_CONTENTS_CURRENT_90)) & ~_CONTENTS_MONSTER, m_collision_group);
		}
